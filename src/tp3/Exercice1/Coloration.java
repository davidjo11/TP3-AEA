...
public class Coloration {
	...
	public void runNaif(Graph g) throws NotEnoughColorsException{
		...
		//Tableau qui pour le sommet i, contient dans voisins[i], l'ensemble des sommets adjacents à celui-ci.
		Edge[][] graph = new Edge[vertex.size()][1];
		//Pour le sommet n°i, si vColor[i][j] (j étant le rgb d'un couleur) est égal à true alors un des voisins du sommet i est coloré par la couleur j.
		...
		while(itv.hasNext()){
			v = itv.next();
			graph[v.getVertex()-1] = g.getEdge(v.getVertex()).toArray(graph[v.getVertex()-1]);
		}
		...
		while(itv.hasNext()){
			...
			while(mincolor<Tools.colorMax && vColor[v.getVertex()-1][mincolor]){
				//vColor[v.getVertex()][i] == true alors un voisin de aux est coloré par la couleur mincolor.
				mincolor += 1;
			}
			...
			for(int j=0;j<graph[aux].length;j++){
				if(graph[aux][j].getVertex()[0] == v){
					voisin = graph[aux][j].getVertex()[1];
					//On indique pour le voisin voisin non coloré de v, qu'une couleur a été ajouté si vColor[voisin.getVertex()][mincolor] == false.
					if(!vColor[voisin.getVertex()-1][mincolor] && voisin.getColor() == -1){
						vColor[voisin.getVertex()-1][mincolor] = true;
					}//Si le voisin n'est pas coloré et que vColor[voisin.getVertex()][mincolor] == false ou si le voisin est coloré alors on ne fait rien.
				}else{
					voisin = graph[aux][j].getVertex()[0];
					//On indique pour le voisin v non coloré de x, qu'une couleur a été ajouté si vColor[v.getVertex()][mincolor] == false.
					if(!vColor[voisin.getVertex()-1][mincolor] && voisin.getColor() == -1){
						vColor[voisin.getVertex()-1][mincolor] = true;
					}//Si le voisin n'est pas coloré et que vColor[voisin.getVertex()][mincolor] == false ou si le voisin est coloré alors on ne fait rien.					}
				}
	 ...
	 * 3 : Choisir un sommet x non encore colorié ayant un DSAT max (si plusieurs sommets sont disponibles alors choisir celui qui a le deg max)
	 * 4 : Colorier x avec la plus petite couleur possible
	 * 5 : Répéter 3 jusqu'à ce que tous les sommets soit coloriés
	 */
	public void runDSATUR(Graph g) throws NotEnoughColorsException{
		...
		itv = vertex.iterator();
		//Construction d'un tableau à 2 dimensions, voisins[i] contient un tableau contenant l'ensemble des edges ayant le vertex n°i pour sommet.
		while(itv.hasNext()){
			v = itv.next();
		...
		//2 (on mettra une couleur au hasard histoire que le sommet soit coloré)
		vertex.get(0).setColor(0);

		//Valeur de aux courante : numéro du sommet de deg max.
		int aux = vertex.get(0).getVertex()-1;
		//On met le degré de saturation de chaque voisin de vertex.get(0) à 1 car on vient de le colorier.
		for(i=0; i<voisins[aux].length;i++){
			if(voisins[aux][i].getVertex()[0].getVertex() == aux+1){
				v = voisins[aux][i].getVertex()[1];
				//On indique pour le voisin v de vertex.get(0) qu'une couleur a été ajouté.
				v.setTag(1);
		...
		while(vertex.size() > 0){
			//Tri des sommets en fonction du degré de saturation dans l'ordre décroissant (voir VertexDsaturComparator).
			...
			//alors aucun de ces voisins n'est coloré, du coup on le colore avec la plus petite couleur (Black => rgb = 0 = minColor).
			if(x.getDegree() == x.getTag())
				...
				while(mincolor<Tools.colorMax && vColor[aux][mincolor]){
					//vColor[aux][mincolor] == true alors un voisin de aux est coloré par la couleur mincolor.
					mincolor += 1;
				}
				if(mincolor > Tools.colorMax){
					throw new NotEnoughColorsException();
				}
				//Mise à jour du tableau des couleurs pour les voisins du sommet x
				for(int j=0;j<voisins[aux].length;j++){
					if(voisins[aux][j].getVertex()[0] == x){
						...
						if(!vColor[v.getVertex()-1][mincolor] && v.getColor() == -1){
							//On augmente le degré de saturation pour le prochain tour.
							v.setTag(v.getTag()+1);
							vColor[v.getVertex()-1][mincolor] = true;
						}//Si le voisin n'est pas coloré et que vColor[v.getVertex()][mincolor] == false ou si le voisin est coloré alors on ne fait rien.
					}else{
	...
	public static void main(String[] args) {
		...
		try{
			Graph g = new ErdosRenyi().generateErdosRenyiGraph(10, (float) 0.5);
			PrintWriter pw;
